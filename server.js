...
const partnerOf = new Map();   // socketId -> partnerId
const profiles = new Map();    // socketId -> { gender, seeking, blocked:Set }
const rate = new Map();        // msg rate limiter
const transcripts = new Map(); // roomId -> [ {type, from?, text, ts} ]
const roomOf = new Map();      // socketId -> roomId

/* ===== Persistence (sync) ===== */
const DATA_DIR = path.join(__dirname, 'data');
try { fs.mkdirSync(DATA_DIR, { recursive: true }); } catch {}
const PATH_REPORTS = path.join(DATA_DIR, 'reports.json');
const PATH_BANS = path.join(DATA_DIR, 'bans.json');
